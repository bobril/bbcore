...
function parseIdentificator() {
    ...
    if (curToken >= 65 && curToken <= 90 || curToken >= 97 && curToken <= 122 || curToken === 95) {
        ...
        if (identificator.charCodeAt(0) == 47 && identificator.charCodeAt(identificator.length - 1) == 47) return buildError("Slash could be only on one side of number");
    ...
    if (identificator[0] <= "9") {
        if (isCloseBracketToken()) {
            ...
            if (identificator[identificator.length - 1] == "/") {
                return {
                    type: "el",
                    id: parseInt(identificator.substr(0, identificator.length - 1))
                };
            }
            if (identificator[0] == "/") {
                return {
                    type: "close",
                    id: parseInt(identificator.substr(1, identificator.length - 1))
                };
            }
            return {
                type: "open",
                id: parseInt(identificator)
            };
        }
