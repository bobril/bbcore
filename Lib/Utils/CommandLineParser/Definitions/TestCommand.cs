...
public class TestCommand : CommonParametersBaseCommand
{
    ...
    public CommandLineArgumentString Out { get; } =
        new("filename for test result as JUnit XML", new[] { "-o", "--out" });
    ...
    public CommandLineArgumentString Port { get; } = new("set port for test server to listen to (default: first free)",
        new[] { "-p", "--port" });

    public CommandLineArgumentBoolNullable Localize { get; } = new("create localized resources (default: autodetect)",
        new[] { "-l", "--localize" });

    public CommandLineArgumentString Dir { get; } =
        new("where to just write test bundle", words: new[] { "-d", "--dir" });
    ...
    public CommandLineArgumentEnumValues TypeCheck { get; } =
        new CommandLineArgumentEnumValues("type check", new[] { "-t", "--typecheck" }, new[] { "no", "yes" }, "yes");
}