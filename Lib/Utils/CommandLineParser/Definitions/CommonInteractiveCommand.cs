...
namespace Lib.Utils.CommandLineParser.Definitions
{
    ...
    {
        public CommandLineArgumentString Port { get; } = new CommandLineArgumentString("set port for server to listen to", new[] { "-p", "--port" }, "8080");
        ...
        public CommandLineArgumentString VersionDir { get; } = new CommandLineArgumentString("store all resources except index.html in this directory", new[] { "-v", "--versiondir" });

        public CommandLineArgumentSwitch BindToAny { get; } = new CommandLineArgumentSwitch("allow to receive connections from external computers", new[] { "--bindToAny" });
